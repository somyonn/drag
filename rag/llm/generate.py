...
class LLMClient:
    ...
    def generate_with_timing(self, prompt: str, context: list[RetrievedChunk]) -> tuple[str, float]:
        """Return (answer, llm_request_ms). Measures only the LLM call (or mock synthesis)."""
        timer = Timer()
        answer = self.generate(prompt, context)
        return answer, timer.elapsed_ms()
...
class CloudThenMockLLMClient(LLMClient):
    ...
    def generate(self, prompt: str, context: list[RetrievedChunk]) -> str:
        answer, _ = self.generate_with_timing(prompt, context)
        return answer

    def generate_with_timing(self, prompt: str, context: list[RetrievedChunk]) -> tuple[str, float]:
        timer = Timer()
        try:
            answer = OpenAIChatClient().generate(prompt, context)
            return answer, timer.elapsed_ms()
        except Exception:
            answer = self._mock.generate(prompt, context)
            return answer, timer.elapsed_ms()
