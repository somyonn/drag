...
def summarize_latencies_ms(values: list[float]) -> dict[str, float]:
    ...
    return {
        ...
        # Population stdev: we treat the collected samples as the full set, and
        # it is defined (0.0) even for a single sample.
        "std_ms": round(pstdev(values), 4) if len(values) > 1 else 0.0,
        "min_ms": min(values),
